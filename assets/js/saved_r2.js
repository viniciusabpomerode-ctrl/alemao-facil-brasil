...
const SavedItems = (() => {
  const WORKER = location.hostname === "localhost"
    ? "http://localhost:8787/saved"
  ...
  function apiUrl() {
    ...
    return location.hostname === "localhost"
      ? `http://localhost:8787/saved/${uid}`
      : `/.netlify/functions/saved-items?user=${uid}`;
  }
  ...
  async function save(items) {
    ...
    try {
      await fetch(apiUrl(), { method: "PUT", headers: {"Content-Type":"application/json"}, body: JSON.stringify(items) });
    } catch {}
  }
